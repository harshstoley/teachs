...
// POST /api/student/homework/:id/submit  (text only — existing)
...
// POST /api/student/homework/:id/submit-file  (file + optional text — NEW)
router.post('/homework/:id/submit-file', ...auth, uploadHomework.single('file'), async (req, res) => {
  try {
    const fileUrl        = req.file ? `/uploads/homework/${req.file.filename}` : null;
    const origFilename   = req.file ? req.file.originalname : null;
    const submissionText = req.body.submission_text || '';
    await db.query(
      `UPDATE homework
       SET status='submitted', submission_text=?, file_url=?, original_filename=?, submitted_at=NOW()
       WHERE id=? AND student_id=?`,
      [submissionText, fileUrl, origFilename, req.params.id, req.user.id]
    );
...
router.get('/syllabus', ...auth, async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT id, subject, chapter, topic, subtopic, status
       FROM syllabus
       WHERE student_id = ?
       ORDER BY subject, chapter, topic, subtopic`,
      [req.user.id]
    );
    res.json(rows);
