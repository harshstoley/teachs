...
export default function StudentDashboard() {
  ...
  const darkCard = { background: 'var(--ink2)', borderRadius: 16, padding: 22, border: '1px solid rgba(59,114,245,0.15)' };
        ...
        {active==='Overview' && data && (
          <div>
            ...
            <div className="grid-4" style={{ marginBottom:24 }}>
              {[
                { label:'My Teachers', value:data.teachers?.length||0, icon:'👨‍🏫', bg:'rgba(59,114,245,0.10)', border:'rgba(59,114,245,0.20)' },
            ...
            <div style={card}>
              ...
              <form onSubmit={submitDoubt}>
                <div className="form-group">
                  ...
                  <input value={doubt.subject} onChange={e => setDoubt({...doubt, subject:e.target.value})} className="form-input" placeholder="e.g. Mathematics" required />
                </div>
                <div className="form-group">
                  ...
                  tabData.map(d => (
                    <div key={d.id} style={{ padding:'14px 0', borderBottom:'1px solid var(--border)' }}>
                      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                        ...
                        <span className={`badge ${d.is_answered ? 'badge-emerald' : 'badge-amber'}`}>{d.is_answered ? '✓ Answered' : '⏳ Pending'}</span>
                      </div>
                      <p style={{ fontSize:'0.87rem', color:'var(--text2)', marginBottom: d.answer ? 8 : 0 }}>{d.question}</p>
                      {d.answer && <div style={{ background:'var(--lavender)', borderRadius:8, padding:'10px 14px', fontSize:'0.84rem', color:'var(--sapphire)', borderLeft:'3px solid var(--sky)' }}>{d.answer}</div>}
                    </div>
                  ))
                }
              </div>
                ...
                {tabData.map((item, i) => (
                  <div key={i} style={{ padding:'12px 0', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:8 }}>
                    ...
                    <div style={{ textAlign:'right' }}>
                      ...
                      {item.status && <span className={`badge ${item.status==='present'?'badge-emerald':'badge-error'}`}>{item.status}</span>}
