...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'Doubts') { api.get('/teacher/doubts-for-me').then(r=>setTabData(r.data)).catch(()=>setTabData([])); return; }
    if (activeTab === 'Grade Homework') { api.get('/teacher/homework-submitted').then(r=>setTabData(r.data)).catch(()=>setTabData([])); return; }
    setTabData(null);
  }, [activeTab]);
  ...
  const submitSched = async e => { e.preventDefault(); try { await api.post('/teacher/schedule-class', schedForm); showMsg('Class scheduled!'); setSchedForm({student_id:'',subject:'',day_of_week:1,start_time:'17:00',duration_min:60,class_date:''}); } catch { showMsg('Failed.','error'); } };
  const gradeHw   = async e => { e.preventDefault(); if(!gradeForm.hw_id) return showMsg('Select homework','error'); try { await api.put(`/teacher/homework/${gradeForm.hw_id}/grade`,{grade:gradeForm.grade,remarks:gradeForm.remarks}); showMsg('Graded!'); setGradeForm({hw_id:'',grade:'',remarks:''}); api.get('/teacher/homework-submitted').then(r=>setTabData(r.data)); } catch { showMsg('Failed.','error'); } };
  const answerDoubt = async id => { if(!doubtAnswer[id]) return showMsg('Type an answer first','error'); try { await api.put(`/teacher/doubts/${id}`,{answer:doubtAnswer[id]}); showMsg('Doubt answered!'); api.get('/teacher/doubts-for-me').then(r=>setTabData(r.data)); } catch { showMsg('Failed.','error'); } };
        ...
        {activeTab==='Overview' && data && (
          <div>
            ...
            <div className="grid-2">
              <div style={card}>
                <h4 style={{ marginBottom:16, color:'var(--text)' }}>This Week's Schedule</h4>
              ...
              <div style={card}>
                <h4 style={{ marginBottom:16, color:'var(--text)' }}>Pending Doubts</h4>
                {data.pendingDoubts?.length ? data.pendingDoubts.map(d => (
                  <div key={d.id} style={{ padding:'12px 0', borderBottom:'1px solid var(--border)' }}>
                    <div style={{ fontSize:'0.82rem', fontWeight:600, color:'var(--sapphire)', marginBottom:4 }}>{d.student_name} – {d.subject}</div>
                    <p style={{ fontSize:'0.82rem', color:'var(--text2)', marginBottom:8 }}>{d.question}</p>
                    <div style={{ display:'flex', gap:8 }}>
                      <input value={doubtAnswer[d.id]||''} onChange={e=>setDoubtAnswer({...doubtAnswer,[d.id]:e.target.value})} placeholder="Type answer..." className="form-input" style={{ flex:1, padding:'7px 10px', fontSize:'0.82rem' }}/>
                      <button onClick={()=>answerDoubt(d.id)} style={{ padding:'7px 14px', background:'linear-gradient(135deg, var(--sapphire) 0%, var(--sapphire2) 100%)', color:'white', border:'none', borderRadius:8, cursor:'pointer', fontWeight:700, fontFamily:'var(--font-body)', fontSize:'0.82rem' }}>Answer</button>
                    </div>
                  </div>
                )) : <p style={{ color:'var(--emerald2)', fontSize:'0.875rem' }}>No pending doubts 🎉</p>}
              </div>
            </div>
          </div>
        )}
                  ...
                  <div key={`${s.id}-${i}`} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 0', borderBottom:'1px solid var(--border)', flexWrap:'wrap', gap:8 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:'0.75rem', color:'var(--text3)' }}>Class {s.student_class} · Parent: {s.parent_name||'N/A'}</div>
                      </div>
                    </div>
        ...
        {activeTab==='Schedule Class' && (
          <div style={{ ...card, maxWidth:540 }}>
            ...
            <form onSubmit={submitSched}>
              ...
              <div style={formGrid}>
                <div className="form-group"><label className="form-label">Class Date</label><input type="date" value={schedForm.class_date} onChange={e=>setSchedForm({...schedForm,class_date:e.target.value})} className="form-input"/></div>
                ...
                <div className="form-group">
                  ...
                  <select value={schedForm.day_of_week} onChange={e=>setSchedForm({...schedForm,day_of_week:e.target.value})} className="form-input">
                    {['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'].map((d,i)=><option key={i+1} value={i+1}>{d}</option>)}
                  </select>
                </div>
              ...
              <button type="submit" style={{ background:'linear-gradient(135deg, var(--sapphire) 0%, var(--sapphire2) 100%)', color:'white', fontWeight:700, padding:'12px 24px', borderRadius:10, border:'none', cursor:'pointer', fontFamily:'var(--font-body)', width:'100%' }}>Schedule Class →</button>
            </form>
          </div>
        )}
        ...
        {activeTab==='Homework' && (
          <div style={{ ...card, maxWidth:540 }}>
            ...
            <form onSubmit={submitHw}>
              ...
              <button type="submit" style={{ background:'linear-gradient(135deg, var(--sapphire) 0%, var(--sapphire2) 100%)', color:'white', fontWeight:700, padding:'12px 24px', borderRadius:10, border:'none', cursor:'pointer', fontFamily:'var(--font-body)', width:'100%' }}>Assign Homework →</button>
            </form>
          </div>
        )}

        {/* GRADE HOMEWORK — NEW */}
        {activeTab==='Grade Homework' && (
          <div style={{ display:'grid', gap:20 }}>
            <div style={{ ...card, maxWidth:480 }}>
              <h4 style={{ marginBottom:20, color:'var(--text)' }}>Grade Submitted Homework</h4>
              {!tabData ? <div style={{ display:'flex', justifyContent:'center', padding:24 }}><div className="spinner"/></div> :
               tabData.length===0 ? <p style={{ color:'var(--text3)', fontSize:'0.88rem' }}>No submitted homework yet.</p> : (
                <form onSubmit={gradeHw}>
                  <div className="form-group">
                    <label className="form-label">Select Homework *</label>
                    <select value={gradeForm.hw_id} onChange={e=>setGradeForm({...gradeForm,hw_id:e.target.value})} className="form-input" required>
                      <option value="">-- Choose Homework --</option>
                      {tabData.map(hw=><option key={hw.id} value={hw.id}>{hw.student_name} – {hw.title}</option>)}
                    </select>
                  </div>
                  <div className="form-group"><label className="form-label">Grade</label><input value={gradeForm.grade} onChange={e=>setGradeForm({...gradeForm,grade:e.target.value})} className="form-input" placeholder="e.g. A, B+, 85%"/></div>
                  <div className="form-group"><label className="form-label">Remarks</label><textarea value={gradeForm.remarks} onChange={e=>setGradeForm({...gradeForm,remarks:e.target.value})} rows={3} className="form-input" placeholder="Feedback for student..."/></div>
                  <button type="submit" style={{ background:'linear-gradient(135deg, var(--emerald) 0%, var(--emerald2) 100%)', color:'white', fontWeight:700, padding:'12px 24px', borderRadius:10, border:'none', cursor:'pointer', fontFamily:'var(--font-body)', width:'100%' }}>Submit Grade →</button>
                </form>
              )}
            </div>
            {tabData && tabData.length>0 && (
              <div style={card}>
                <h4 style={{ marginBottom:16, color:'var(--text)' }}>Submitted ({tabData.length})</h4>
                {tabData.map(hw=>(
                  <div key={hw.id} style={{ padding:'10px 0', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:8 }}>
                    <div><div style={{ fontWeight:600, fontSize:'0.88rem', color:'var(--text)' }}>{hw.student_name} – {hw.title}</div>{hw.submission_text&&<p style={{ fontSize:'0.78rem', color:'var(--text3)', marginTop:3 }}>{hw.submission_text?.slice(0,80)}...</p>}</div>
        ...
        {activeTab==='Progress' && (
          <div style={{ ...card, maxWidth:540 }}>
            ...
            <form onSubmit={submitProg}>
              ...
              <button type="submit" style={{ background:'linear-gradient(135deg, var(--sapphire) 0%, var(--sapphire2) 100%)', color:'white', fontWeight:700, padding:'12px 24px', borderRadius:10, border:'none', cursor:'pointer', fontFamily:'var(--font-body)', width:'100%' }}>Save Report →</button>
            </form>
          </div>
        )}
        ...
        {activeTab==='Attendance' && (
          <div style={{ ...card, maxWidth:480 }}>
            ...
            <form onSubmit={submitAtt}>
              ...
              <button type="submit" style={{ background:'linear-gradient(135deg, var(--sapphire) 0%, var(--sapphire2) 100%)', color:'white', fontWeight:700, padding:'12px 24px', borderRadius:10, border:'none', cursor:'pointer', fontFamily:'var(--font-body)', width:'100%' }}>Mark Attendance →</button>
            </form>
          </div>
        )}

        {/* DOUBTS — from students — NEW */}
        {activeTab==='Doubts' && (
          <div style={card}>
            <h4 style={{ marginBottom:20, color:'var(--text)' }}>Doubts From My Students</h4>
            {!tabData ? <div style={{ display:'flex', justifyContent:'center', padding:32 }}><div className="spinner"/></div> :
                  ...
                  <div key={d.id} style={{ padding:'16px 0', borderBottom:'1px solid var(--border)' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8, flexWrap:'wrap', gap:6 }}>
                      <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                        ...
                        <span className="badge badge-sapphire" style={{ fontSize:'0.68rem' }}>{d.subject||'General'}</span>
                      </div>
                      <span className={`badge ${d.status==='answered'?'badge-emerald':'badge-amber'}`}>{d.status==='answered'?'✓ Answered':'⏳ Pending'}</span>
                    </div>
                    <p style={{ fontSize:'0.87rem', color:'var(--text)', fontWeight:500, marginBottom:d.status==='pending'?12:0 }}>{d.question}</p>
                    {d.answer && <div style={{ background:'var(--lavender)', borderRadius:8, padding:'10px 14px', fontSize:'0.84rem', color:'var(--sapphire)', borderLeft:'3px solid var(--sky)', marginBottom:8 }}><strong>Your Answer:</strong> {d.answer}</div>}
                    {d.status==='pending' && (
                      <div style={{ display:'flex', gap:8 }}>
                        <input value={doubtAnswer[d.id]||''} onChange={e=>setDoubtAnswer({...doubtAnswer,[d.id]:e.target.value})} placeholder="Type your answer..." className="form-input" style={{ flex:1 }}/>
                        <button onClick={()=>answerDoubt(d.id)} style={{ padding:'10px 20px', background:'linear-gradient(135deg, var(--sapphire) 0%, var(--sapphire2) 100%)', color:'white', border:'none', borderRadius:10, fontWeight:700, cursor:'pointer', fontFamily:'var(--font-body)', whiteSpace:'nowrap' }}>Answer</button>
                      </div>
                    )}
                  </div>
                ))}
        ...
        {activeTab==='Notes' && (
          <div style={{ ...card, maxWidth:540 }}>
            ...
            <form onSubmit={submitNote}>
              ...
              <button type="submit" style={{ background:'linear-gradient(135deg, var(--sapphire) 0%, var(--sapphire2) 100%)', color:'white', fontWeight:700, padding:'12px 24px', borderRadius:10, border:'none', cursor:'pointer', fontFamily:'var(--font-body)', width:'100%' }}>Save Notes →</button>
            </form>
          </div>
        )}
        ...
        {activeTab==='Feedback' && (
          <div style={{ ...card, maxWidth:480 }}>
            ...
            <form onSubmit={submitFb}>
              ...
              <button type="submit" style={{ background:'linear-gradient(135deg, var(--sapphire) 0%, var(--sapphire2) 100%)', color:'white', fontWeight:700, padding:'12px 24px', borderRadius:10, border:'none', cursor:'pointer', fontFamily:'var(--font-body)', width:'100%' }}>Submit Feedback →</button>
            </form>
          </div>
        )}
      </div>
