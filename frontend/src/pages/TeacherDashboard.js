...
// dayOfWeek: 1=Mon..7=Sun (DB convention)
...
function ScheduleTab({ students, showMsg }) {
  ...
  const [, setTick] = useState(0); // clock tick for Join button
          ...
          schedules.map(s => (
            <div key={s.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 0', borderBottom:'1px solid var(--border)', flexWrap:'wrap', gap:8 }}>
              ...
              <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
                {/* Join Class button — only when it's class time */}
                {s.meet_link && isClassJoinable(s.day_of_week, s.start_time, s.duration_min) && (
                  <a
                    href={s.meet_link}
                    target="_blank"
                    rel="noreferrer"
                    style={{ padding:'5px 14px', background:'#0099B2', color:'white', borderRadius:7, fontWeight:700, fontSize:'0.78rem', textDecoration:'none', fontFamily:'var(--font-body)', whiteSpace:'nowrap' }}
                  >
                    🔴 Join Class
                  </a>
                )}
