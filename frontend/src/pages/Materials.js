    ...
    <div style={{ background:'var(--navy)', minHeight:'100vh' }}>
      ...
      <section style={{ paddingTop:'calc(var(--nav-height) + 60px)', paddingBottom:52, position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:1100, margin:'0 auto', padding:'0 20px', position:'relative', textAlign:'center' }}>
          ...
          <p style={{ color:'var(--slate2)', maxWidth:480, margin:'0 auto 24px', lineHeight:1.75 }}>Free notes, question papers, worksheets, and sample papers for Class 1–10. Prepared by our expert educators.</p>
          {!user && (
            <div style={{ display:'inline-flex', alignItems:'center', gap:12, background:'rgba(212,168,83,0.08)', border:'1px solid rgba(212,168,83,0.15)', borderRadius:12, padding:'12px 20px', flexWrap:'wrap', justifyContent:'center' }}>
              <span style={{ color:'var(--slate2)', fontSize:'0.85rem' }}>📥 Login to download materials</span>
                ...
                <div style={{ padding:'18px 16px', flex:1, display:'flex', flexDirection:'column' }}>
                  ...
                  {m.file_url ? (
                    user ? (
                      ...
                      <Link to="/login" style={{ display:'block', width:'100%', padding:'10px', background:'rgba(212,168,83,0.12)', color:'var(--gold)', borderRadius:10, fontWeight:700, fontSize:'0.85rem', textAlign:'center', textDecoration:'none', boxSizing:'border-box' }}>
                        Login to Download
                      </Link>
                    )
                  ) : (
